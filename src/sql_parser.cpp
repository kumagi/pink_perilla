...
bool ConsumeKeyword(std::string_view& sv, const std::string& keyword) {
    ...
    if (sv.length() >= keyword.length() &&
        (sv.length() == keyword.length() || !std::isalnum(sv[keyword.length()]))) {
...
absl::StatusOr<std::string> ParseType(std::string_view& sv) {
    ConsumeWhitespace(sv);
    auto type_name_status = ParseIdentifier(sv);
    if (!type_name_status.ok()) return type_name_status.status();
    std::string type_name = *type_name_status;
    ...
    if (consume_char(sv, '(')) {
        ...
        if (paren_level == 0) {
            ...
            return absl::InvalidArgumentError("Mismatched parentheses in type definition");
        }
    }
    return full_type;
}
...
absl::StatusOr<ColumnDef> ParseColumnDef(std::string_view& sv) {
    ...
    if (!name_status.ok()) return name_status.status();

    auto type_status = ParseType(sv);
    if (!type_status.ok()) return type_status.status();
...
absl::StatusOr<CreateTableInfo> ParseCreateTable(std::string_view sql) {
    ...
    if (!table_name_status.ok()) return table_name_status.status();
    ...
    } while (consume_char(sql, ','));

    if (!consume_char(sql, ')'))
        return absl::InvalidArgumentError("Expected ')' after column definitions");
...
absl::StatusOr<DropTableInfo> ParseDropTable(std::string_view sql) {
    ...
    ConsumeWhitespace(sql);
    if (!sql.empty())
        return absl::InvalidArgumentError("Unexpected characters after table name");
...
absl::StatusOr<UpdateInfo> ParseUpdateStatement(std::string_view sql) {
    ...
    do {
        ...
        if (!column_name_status.ok()) return column_name_status.status();
    ...
    if (ConsumeKeyword(sql, "WHERE")) {
        ...
        ConsumeWhitespace(sql);
        if (!sql.empty())
            return absl::InvalidArgumentError("Unexpected characters after SET clause");
    }
...
absl::StatusOr<InsertInfo> ParseInsertStatement(std::string_view& sql) {
    ...
    if (!consume_char(sql, '(')) return absl::InvalidArgumentError("Expected '(' after table name");

    std::vector<std::string> columns;
    do {
        auto col_status = ParseIdentifier(sql);
        if (!col_status.ok()) return col_status.status();
        columns.push_back(*col_status);
    } while (consume_char(sql, ','));

    if (!consume_char(sql, ')')) return absl::InvalidArgumentError("Expected ')' after column list");

    if (!ConsumeKeyword(sql, "VALUES")) return absl::InvalidArgumentError("Expected 'VALUES'");

    if (!consume_char(sql, '(')) return absl::InvalidArgumentError("Expected '(' before values");

    std::vector<std::string> values;
    do {
        ConsumeWhitespace(sql);
        size_t value_end = sql.find_first_of(",)");
        if (value_end == std::string_view::npos) return absl::InvalidArgumentError("Unterminated values list");
        values.push_back(std::string(sql.substr(0, value_end)));
        sql.remove_prefix(value_end);
    } while (consume_char(sql, ','));

    if (!consume_char(sql, ')')) return absl::InvalidArgumentError("Expected ')' after values");
...
absl::StatusOr<SelectItem> ParseSelectItem(std::string_view& sv) {
    ...
    if (!id1_status.ok()) return id1_status.status();
    ...
    if (consume_char(sv, '(')) {
        ...
        if (!consume_char(sv, ')')) {
            ...
            if (!arg_id_status.ok()) return arg_id_status.status();
            arg_id = *arg_id_status;
            if (!consume_char(sv, ')')) return absl::InvalidArgumentError("Expected ')' after function argument");
        }
        ...
        if (ConsumeKeyword(sv, "OVER")) {
            if (!consume_char(sv, '(')) return absl::InvalidArgumentError("Expected '(' after OVER");
            ...
            } while (consume_char(sv, ','));

            if (!consume_char(sv, ')')) return absl::InvalidArgumentError("Expected ')' after PARTITION BY clause");

            size_t full_len = original_sv.length() - sv.length();
            std::string full_expr = std::string(original_sv.substr(0, full_len));
            ConsumeWhitespace(sv);
    ...
    if (ConsumeKeyword(sv, "AS")) {
        ...
        if (!alias_status.ok()) return alias_status.status();
        expression += " AS " + *alias_status;
    }

    ConsumeWhitespace(sv);
    return SelectItem{SelectItemType::COLUMN, expression, std::nullopt, std::nullopt};
}
...
absl::StatusOr<SelectInfo> ParseSelectStatement(std::string_view& sql) {
    ...
    if (consume_char(sql, '*')) {
        select_items.push_back({SelectItemType::COLUMN, "*", std::nullopt, std::nullopt});
    } else {
        auto first_item_status = ParseSelectItem(sql);
        if (!first_item_status.ok()) return first_item_status.status();
        select_items.push_back(*first_item_status);

        while (consume_char(sql, ',')) {
            auto next_item_status = ParseSelectItem(sql);
            if (!next_item_status.ok()) return next_item_status.status();
    ...
    if (consume_char(sql, '(')) {
        ...
        if (!subquery_status.ok()) return subquery_status.status();
        if (!consume_char(sql, ')')) return absl::InvalidArgumentError("Expected ')' after subquery");
        result_info.from_subquery = std::make_unique<SelectInfo>(std::move(*subquery_status));
    } else {
        auto table_name_status = ParseIdentifier(sql);
        if (!table_name_status.ok()) return table_name_status.status();
    ...
    while (consume_char(sql, ',')) {
        ...
        if (!next_table_status.ok()) return next_table_status.status();
    ...
    while (true) {
        ...
        if (!join_table_status.ok()) return join_table_status.status();
        ...
        result_info.joins.push_back({join_type, *join_table_status, on_condition});
    }

    if (ConsumeKeyword(sql, "GROUP") && ConsumeKeyword(sql, "BY")) {
        do {
            auto col_name_status = ParseIdentifier(sql);
            if (!col_name_status.ok()) return col_name_status.status();
    ...
    if (ConsumeKeyword(sql, "ORDER") && ConsumeKeyword(sql, "BY")) {
        do {
            ...
            if (ConsumeKeyword(sql, "DESC")) {
                sort_info.direction =
                    substrait::SortField::SORT_DIRECTION_DESC_NULLS_FIRST;
            } else {
                ConsumeKeyword(sql, "ASC");
                sort_info.direction =
                    substrait::SortField::SORT_DIRECTION_ASC_NULLS_FIRST;
            }
    ...
    if (ConsumeKeyword(sql, "LIMIT")) {
        ...
        if (i > 0) {
            ...
            if (!limit_val_status)
                return absl::InvalidArgumentError("Failed to parse LIMIT value");
            result_info.limit = *limit_val_status;
            sql.remove_prefix(i);
        } else {
            return absl::InvalidArgumentError("LIMIT must be followed by a number");
        }
    }
...
absl::StatusOr<substrait::Plan> ParseSql(std::string_view sql) {
    ...
    ConsumeWhitespace(temp_sql);
    TrimRight(temp_sql);
    if (auto select_status = ParseSelectStatement(temp_sql); select_status.ok()) {
        if (select_status.ok()) {
            return pink_perilla::converter::ToSubstrait(*select_status);
        }
    }
    ...
    if (auto create_status = ParseCreateTable(temp_sql); create_status.ok()) {
        if (create_status.ok()) {
            return pink_perilla::converter::ToSubstrait(*create_status);
        }
    }
    ...
    if (auto drop_status = ParseDropTable(temp_sql); drop_status.ok()) {
        if (drop_status.ok()) {
            return pink_perilla::converter::ToSubstrait(*drop_status);
        }
    }
    ...
    ConsumeWhitespace(temp_sql);
    TrimRight(temp_sql);
    if (auto delete_status = ParseDeleteStatement(temp_sql); delete_status.ok()) {
        if (delete_status.ok()) {
            return pink_perilla::converter::ToSubstrait(*delete_status);
        }
    }
    ...
    ConsumeWhitespace(temp_sql);
    TrimRight(temp_sql);
    if (auto update_status = ParseUpdateStatement(temp_sql); update_status.ok()) {
        if (update_status.ok()) {
            return pink_perilla::converter::ToSubstrait(*update_status);
        }
    }
    ...
    ConsumeWhitespace(temp_sql);
    TrimRight(temp_sql);
    if (auto insert_status = ParseInsertStatement(temp_sql); insert_status.ok()) {
        if (insert_status.ok()) {
            return pink_perilla::converter::ToSubstrait(*insert_status);
        }
    }

    return absl::InvalidArgumentError("Failed to parse SQL statement or statement not supported.");
}
